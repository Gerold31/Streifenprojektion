...
Bei der Triangulation geht es darum, aus zwei Eckpunkten und den Innenwickeln eines Dreiecks den dritten Eckpunkt zu bestimmen. In diesem Projekt geht es darum, den Objektpunkt auf Basis eines Bildpunktes und der Laserausrichtung zu bestimmen.

\subsubsection{Modell und gegebene Werte}

Der Sachverhalt wird dabei zunächst auf das folgende Modell abstrahiert: Es existiert eine perspektivischen Projektion eines Objektpunktes $X$ auf eine Bildebene $B$, die parallel zur $x$-$y$-Ebene verläuft. Der Augpunkt $O$ liegt dabei im Koordinatenursprung. Der Bildpunkt wird mit $X'$ bezeichnet. Das projizierte Licht wird durch eine Ebene $L$ dargestellt, die die Position des Projektors, das heißt den Projektionspunkt $P$, und den Objektpunkt $X$ schneidet.

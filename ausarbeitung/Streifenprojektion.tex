...
Das Ergebnis ist ein Binärbild, mit dessen Hilfe im zweiten Schritt das \texttt{Line}-Objekt gefüllt wird. Dafür werden die Pixel das Binärbildes zeilenweise durchlaufen. Sobald ein Pixel gefunden wurde, der vermeidlich zur Linie gehört, wird dieser mit allen anschließenden markierten Pixel in der selben Zeile als ein Punkt auf Linie betrachtet. Die Position wird als Abtastung der Linie aufgenommen, wobei die Anzahl der hintereinander als Linie markierten Pixel als die Breite der Linie aufgefasst wird.
...
Anstatt einen Mittelwerte über dem gesamten Bild zu berechnen, wird für jeden Pixel ein eigener Mittelwert über einer eigenen rechteckige Umgebung bestimmt.
Zusammen mit der Farbe des Pixels $c$ wird aus dem Mittelwert $m$ eine Beurteilung $e \in [0, 1]$ des Punktes berechnet. Um so größer $e$ ist, desto wahrscheinlicher soll der Pixel zur Linie gehören. Ist $e$ größer als ein festgelegter Schwellenwert (0.6), wird davon ausgegangen, dass der Punkt zur Linie gehört.
Zur Berechnung von $e$ werden dabei zunächst die Zwischenergebnisse $r_{r/g/b}$ berechnet, um diese dann gewichtet in $e$ einzubringen:
\begin{align*}
	r_i &= \begin{cases}

...
\begin{tabular}{lp{12cm}}
	...
	$\theta$,$\delta$  &
		Winkel zur Ausrichtung der Ebene $L$. Die Ausgangslage der Ebene ist parallel zur $y$-$z$-Ebene. Die Ebene wird zunächst mit dem Winkel $\theta$ um den Vektor {\color{red} $(0,0,-1)^T$} gedreht. Danach mit dem Winkel $\delta$ um den Vektor {\color{red} $(-\sin(\theta), -\cos(\theta), 0)^T$}. Siehe auch Abbildung~\ref{fig:triangulation_skew_pitch}.
\end{tabular}
...
Die Ergebnisse zeigen, dass die Linienerkennung 'diff' Probleme mit spiegelnden Oberflächen hat.
